...
int main() {
  ...
  CC(); 
  memset(magic, '0', sizeof(magic)-1); //2 rip
  CC(); 
  if(oracle_connect() < 0) errquit("connect"); //3 record rip and magic, set magic 
  CC(); 
  if(oracle_reset() < 0)   errquit("reset"); //4  //1
  CC(); 
  oracle_update(0x5bed1dcc); //5  //2
  if(magic[0] == '0') {
